...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	{
		// 绘画牌精 文字
		TCHAR szBuffer[32] = TEXT("牌精");
		DrawTextString( pDC,szBuffer,RGB(255,255,255),RGB(0,0,0),m_nXBorder+5+56+130,m_nYBorder+5+12,true );
		// 绘画牌精 牌图片
		g_CardResource.m_ImageUserBottom.DrawCardItem(pDC,m_cbPaiJin,m_nXBorder+5+56+150,m_nYBorder+5);
	}
#endif 
			...
			{	
				...
				if (m_bBombEffect==true)
				{
					...
					if (m_cbUserAction[i]&WIK_PENG) nYImagePos=nItemHeight;
 					...
 					else if (m_cbUserAction[i]&WIK_TIE_PAI) nYImagePos=nItemHeight*3;
					else if (m_cbUserAction[i]&WIK_TI_PAI) nYImagePos=0;
