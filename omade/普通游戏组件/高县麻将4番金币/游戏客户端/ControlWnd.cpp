...
void CControlWnd::SetBenchmarkPos(int nXPos, int nYPos)
{
 ...
 //设置扑克
void CControlWnd::SetControlInfo(BYTE cbCenterCard, BYTE cbActionMask, tagGangCardResult & GangCardResult,tagTICardResult & TICardResult,BYTE cbMagicCard)
{
	//设置变量
	m_cbItemCount=0;
	m_cbCurrentItem=0xFF;
	...
	for (BYTE i=0;i<GangCardResult.cbCardCount;i++) 
	{
 	...
 	//提牌信息
	ZeroMemory(m_cbTICard,sizeof(m_cbTICard));
	for (BYTE i=0;i<TICardResult.cbCardCount;i++) 
	{
		m_cbItemCount++;
		m_cbTICard[i]=TICardResult.cbCardData[i];
 	}

 
	//计算数目
 	BYTE cbItemKind[2]={WIK_TIE_PAI,WIK_PENG};
...
void CControlWnd::OnPaint()
{
	...
	BYTE cbExcursion[3]={0,1,2};
 	...
 	BYTE cbItemKind[2]={WIK_TIE_PAI,WIK_PENG};
 	BYTE tmpAction = m_cbActionMask;
	//绘画扑克
	for (BYTE i=0;i<CountArray(cbItemKind);i++)
	{
		if ((tmpAction&cbItemKind[i])!=0)
		{
			BYTE cbCardData [3]= {0,0,0};
			if ((tmpAction&WIK_TIE_PAI)!=0 )
			{
 				cbCardData[0] = m_cbMagicCard;
				cbCardData[1] = m_cbCenterCard;
				cbCardData[2] = m_cbCenterCard;
				tmpAction &= ~WIK_TIE_PAI;
			}else if ((tmpAction&WIK_PENG)!=0)
			{
 
				cbCardData[0] = m_cbCenterCard;
				cbCardData[1] = m_cbCenterCard;
				cbCardData[2] = m_cbCenterCard;
				tmpAction &= ~WIK_PENG;
			}
			...
			for (BYTE j=0;j<3;j++)
			{
 				g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,cbCardData[j],j*26+12,nYPos+5);
			}

			//计算位置
			int nXImagePos=0;
			int nItemWidth=m_ImageActionExplain.GetWidth()/7; // 7 是   7 个字
			if ((m_cbActionMask&cbItemKind[i])&WIK_PENG) // 碰是第2个字，起点位置是一个字宽
				nXImagePos=nItemWidth;
			if ((m_cbActionMask&cbItemKind[i])&WIK_TIE_PAI) // 碰是第2个字，起点位置是一个字宽
				nXImagePos=nItemWidth*2;
 	...
 	//提牌扑克
	for (BYTE i=0;i<CountArray(m_cbTICard);i++)
	{
 		if (m_cbTICard[i]!=0)
		{
			//绘画扑克
			for (BYTE j=0;j<3;j++)
			{
				BYTE cbCardData = (j==0)?m_cbMagicCard:m_cbTICard[i];
			...
			int nItemHeight=m_ImageActionExplain.GetHeight();
			m_ImageActionExplain.BitBlt(BufferDC,126,nYPos+5,nItemWidth,nItemHeight,nItemWidth*5,0);
...
void CControlWnd::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_cbCurrentItem!=0xFF)
	{
		...
		for (BYTE i=0;i<CountArray(m_cbTICard);i++)
		{
			if ((m_cbTICard[i]!=0)&&(m_cbCurrentItem==cbIndex++))
			{
				AfxGetMainWnd()->PostMessage(IDM_CARD_OPERATE,WIK_TI_PAI,m_cbTICard[i]);
				return;
			}
		}
