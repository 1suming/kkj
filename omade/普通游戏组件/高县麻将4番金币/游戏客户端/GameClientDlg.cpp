...
//////////////////////////////////////////////////////////////////////////
			...
			if ((IsLookonMode()==false)&&(pStatusPlay->cbActionMask!=WIK_NULL))
			{
				...
				//提牌判断
				if ((cbActionMask&WIK_TI_PAI)!=0)
				{
					WORD wMeChairID=GetMeChairID();
					m_GameLogic.AnalyseTIPaiCard(m_cbCardIndex,m_WeaveItemArray[wMeChairID],m_cbWeaveCount[wMeChairID],TICardResult);
				}
				...
				if (IsLookonMode()==false) 
 					m_GameClientView.m_ControlWnd.SetControlInfo(cbActionCard,cbActionMask,GangCardResult,TICardResult,m_GameLogic.SwitchToCardData(m_GameLogic.GetMagicIndex()));
 			}
...
bool CGameClientDlg::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	//摇色子动画
	m_GameClientView.m_DrawSiceWnd.SetSiceInfo(m_GameClientView.GetDC(),200,
	(WORD)(pGameStart->lSiceCount>>16),(WORD)pGameStart->lSiceCount,pGameStart->bKaiJu);
...
bool CGameClientDlg::OnSubOutCard(const void * pBuffer, WORD wDataSize)
{
	...
	{
		// 当前用户如果打听用，设置为托管不可改变
		if(m_GameLogic.IsMagicCard(m_cbOutCardData)&&m_bOutMagicCard==false)
		{
			...
			m_bStustee = false;
			OnStusteeControl(0,0);
			m_GameClientView.m_btStusteeControl.EnableWindow(FALSE);
 		}
	}
...
bool CGameClientDlg::OnSubSendCard(const void * pBuffer, WORD wDataSize)
{
	...
	if ((IsLookonMode()==false)&&(m_wCurrentUser==wMeChairID))
	{
		...
		if (pSendCard->cbActionMask!=WIK_NULL)
		{
			...
			{
				WORD wMeChairID=GetMeChairID();
				m_GameLogic.AnalyseTIPaiCard(m_cbCardIndex,m_WeaveItemArray[wMeChairID],m_cbWeaveCount[wMeChairID],TIResult);
			}

 			//设置界面
			m_GameClientView.m_ControlWnd.SetControlInfo(cbActionCard,cbActionMask,GangCardResult,TIResult,m_GameLogic.SwitchToCardData(m_GameLogic.GetMagicIndex()));
  		}
	}
...
bool CGameClientDlg::OnSubOperateNotify(const void * pBuffer, WORD wDataSize)
{
	...
	CMD_S_OperateNotify * pOperateNotify=(CMD_S_OperateNotify *)pBuffer;
 	if (m_cbLeftCardCount<4 && ((pOperateNotify->cbActionMask&WIK_CHI_HU))!=0)
	{
		// 最后4张内，有胡牌，自动胡牌
		OnCardOperate(WIK_CHI_HU,pOperateNotify->cbActionCard);
		return true;
	}
...
bool CGameClientDlg::OnSubOperateResult(const void * pBuffer, WORD wDataSize)
{
	...
	else if (pOperateResult->cbOperateCode!=WIK_NULL)
	{
		...
		if (pOperateResult->cbOperateCode&WIK_TI_PAI) // 提牌
		{
 			for (BYTE kk = 0; kk< m_cbWeaveCount[wOperateUser];kk++)
			{
				
				BYTE cbCenterCard = m_WeaveItemArray[wOperateUser][kk].cbCenterCard;

				if(m_WeaveItemArray[wOperateUser][kk].cbWeaveKind == WIK_TIE_PAI &&(cbOperateCard == cbCenterCard)) // 贴牌
				{
					cbWeaveIndex = kk;
					m_WeaveItemArray[wOperateUser][cbWeaveIndex].cbPublicCard=TRUE;
 					m_WeaveItemArray[wOperateUser][cbWeaveIndex].cbWeaveKind=WIK_PENG;
 					break;
				}
			}
		}else
		...
		if (GetMeChairID()==wOperateUser)
		{
 			if(!(pOperateResult->cbOperateCode&WIK_TI_PAI))
			{
		...
		if (GetMeChairID()==wOperateUser)
		{
 			...
 			if(pOperateResult->cbOperateCode&WIK_TI_PAI)
			{
 				...
 				// 删除替换扑克
				m_GameLogic.RemoveCard(m_cbCardIndex,cbOperateCard);
				// 提回听用扑克
				m_cbCardIndex[m_GameLogic.GetMagicIndex()]++;

				cbCardCount=m_GameLogic.SwitchToCardData(m_cbCardIndex,cbCardData);
				m_GameClientView.m_HandCardControl.SetCardData(cbCardData,cbCardCount-1,cbCardData[cbCardCount-1]);
  			}
			else if(pOperateResult->cbOperateCode&WIK_TIE_PAI)
			{
 				cbCardCount=m_GameLogic.SwitchToCardData(m_cbCardIndex,cbCardData);
				m_GameClientView.m_HandCardControl.SetCardData(cbCardData,cbCardCount-1,cbCardData[cbCardCount-1]);
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	//删除定时器
	KillGameTimer(IDI_OPERATE_CARD);
 	m_GameClientView.m_cbPaiJin = 0xFF;
	m_bOutMagicCard = false;
	m_GameClientView.m_btStusteeControl.EnableWindow(TRUE);
 	//设置控件
	//m_GameClientView.SetStatusFlag(false,false);
	...
	for( WORD i = 0; i < GAME_PLAYER; i++ )
	{
		...
		lstrcpy( ScoreInfo.szUserName[SwitchViewChairID(i)],m_szAccounts[i] );
 		strTemp.Format(TEXT("%s: 番数得分:%ld, 花猪得分:%ld, 查叫得分:%ld 抽税: %ld \n"),
		ScoreInfo.szUserName[SwitchViewChairID(i)],pGameEnd->lGameScore[i],pGameEnd->lHuaZhuScore[i],pGameEnd->lChaJiaoScore[i],pGameEnd->lGameTax[i]);
 		strEnd += strTemp;
	}
	//消息积分
	...
	for( WORD i = 0; i < GAME_PLAYER; i++ )
	{
		...
		if( j == CountArray(pGameEnd->wWinOrder) )
		{
			ScoreInfo.cbWinOrder = 0;
		}
		...
		{
			if( j == i ) continue;
			WORD jviewID = SwitchViewChairID(j);

			ScoreInfo.wLostFanShu[jviewID] = pGameEnd->wLostFanShu[i][j];
		...
		{
			WORD jviewID = SwitchViewChairID(j);
			ScoreInfo.lChaJiaoScore[jviewID] = pGameEnd->lChaJiaoScore[j];
			ScoreInfo.lHuaZhuScore[jviewID] = pGameEnd->lHuaZhuScore[j];
...
bool CGameClientDlg::OnSubUserChiHu( const void *pBuffer,WORD wDataSize )
{
	...
	CMD_S_ChiHu * pChiHu=(CMD_S_ChiHu *)pBuffer;
 	bool bQiangGang = pChiHu->bGangStatus; // 读取抢杠状态
	bool bQiangTI = pChiHu->bTIStatus; // 读取抢提状态
	BYTE cbProvider = pChiHu->wProviderUser;
	BYTE cbChiHuCard = pChiHu->cbChiHuCard;
 	WORD wViewId = SwitchViewChairID( pChiHu->wChiHuUser ); //wViewId 胡牌用户视图ID
 	...
 	if (pChiHu->bGangStatus == true)
	{
		//删除扑克
 		// 改变杠类型为碰类型
		for (BYTE j = 0;j<m_cbWeaveCount[cbProvider];j++)
		{
			BYTE cbCenterCard = m_WeaveItemArray[cbProvider][j].cbCenterCard;
			if (m_WeaveItemArray[cbProvider][j].cbWeaveKind == WIK_GANG && cbCenterCard == cbChiHuCard)
			{
				m_WeaveItemArray[cbProvider][j].cbWeaveKind = WIK_PENG;// 手上牌不变
 				//组合界面
				BYTE cbWeaveCard[4]={0,0,0,0};
				WORD wViewProvider = SwitchViewChairID(cbProvider);
				BYTE cbWeaveCardCount=m_GameLogic.GetWeaveCard(WIK_PENG,cbCenterCard,cbWeaveCard);
				m_GameClientView.m_WeaveCard[wViewProvider][j].SetCardData(cbWeaveCard,cbWeaveCardCount);
				m_GameClientView.m_WeaveCard[wViewProvider][j].SetDisplayItem(true);

				m_GameClientView.SetUserAction(wViewProvider,0);
				break;
			}
		}
	...
	}else if (pChiHu->bTIStatus == true)
	{
		...
		{
			BYTE cbCenterCard = m_WeaveItemArray[cbProvider][j].cbCenterCard;
			if (m_WeaveItemArray[cbProvider][j].cbWeaveKind == WIK_PENG && cbCenterCard == cbChiHuCard)
			{
				m_WeaveItemArray[cbProvider][j].cbWeaveKind = WIK_TIE_PAI;
				m_cbCardIndex[m_GameLogic.GetMagicIndex()]--; // 返回提回听用
				//组合界面
				WORD wViewProvider = SwitchViewChairID(cbProvider);
				BYTE cbWeaveCard[4]={0,0,0,0};
				BYTE cbWeaveCardCount=m_GameLogic.GetWeaveCard(WIK_TIE_PAI,cbCenterCard,cbWeaveCard);
				m_GameClientView.m_WeaveCard[wViewProvider][j].SetCardData(cbWeaveCard,cbWeaveCardCount);
				m_GameClientView.m_WeaveCard[wViewProvider][j].SetDisplayItem(true);
	...
	{
		BYTE cbCenterCard = m_WeaveItemArray[cbProvider][j].cbCenterCard;
		BYTE cbWeaveKind = m_WeaveItemArray[cbProvider][j].cbWeaveKind;
 	}
...
void CGameClientDlg::PlayActionSound(WORD wChairID,BYTE cbAction)
{
	...
	case WIK_LEFT:  //	case WIK_TI_PAI:

		{
			if(bBoy)
				PlayGameSound(AfxGetInstanceHandle(),TEXT("BOY_TI"));
			else
				PlayGameSound(AfxGetInstanceHandle(),TEXT("GIRL_TI"));		
			break;

		}

	case WIK_CENTER: 	//case WIK_TIE_PAI:

		{

			if(bBoy)
				PlayGameSound(AfxGetInstanceHandle(),TEXT("BOY_TIE"));
			else
				PlayGameSound(AfxGetInstanceHandle(),TEXT("GIRL_TIE"));		
			break;
...
bool CGameClientDlg::ShowOperateControl(BYTE cbUserAction, BYTE cbActionCard)
{
	...
	//显示界面
	if (IsLookonMode()==false)
 		m_GameClientView.m_ControlWnd.SetControlInfo(cbActionCard,cbUserAction,GangCardResult,TICardResult,m_GameLogic.SwitchToCardData(m_GameLogic.GetMagicIndex()));
 	return true;
}
...
LRESULT CGameClientDlg::OnOutCard(WPARAM wParam, LPARAM lParam)
{
	...
	m_GameLogic.RemoveCard(m_cbCardIndex,cbOutCardData);
 	...
 	if ((m_GameLogic.IsMagicCard(cbOutCardData))&&(m_GameClientView.m_bEnJoinStustee == false))
	{
		// 自己打出听用牌，设置托管为灰色不可用
		m_GameClientView.m_bEnJoinStustee = true;
		// 设置托管
		OnStusteeControl(TRUE,0); // 发送托管消息到服务器
		m_GameClientView.m_btStusteeControl.EnableWindow(TRUE); // 禁止托管按钮
	}
 ...
 bool CGameClientDlg::OnSubCardPaiJin( const void *pBuffer, WORD wDataSize )
{
 ...
 bool CGameClientDlg::OnSubGrantScore( const void *pBuffer, WORD wDataSize )
 {
	 ...
	 InsertSystemString(msg);

	 ShowInformationEx(msg,0,MB_ICONINFORMATION,TEXT("高县麻将"));
	 return true;
   }
 //////////////////////////////////////////////////////////////////////////
