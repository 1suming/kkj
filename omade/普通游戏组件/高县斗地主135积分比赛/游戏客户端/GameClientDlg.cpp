...
CGameClientDlg::CGameClientDlg() : CGameFrameDlg(&m_GameClientView)
{
	...
	if((freopen("LandGXlog.txt", "w+", stdout)) == NULL)
		exit(-1);
	...
	case IDI_LAND_SCORE:		//挖坑叫分
		{
			if ( nElapse==0 || ( nElapse <= TIME_OUT_CARD && m_bAutoPlay ) )
			{
				if ((IsLookonMode()==false)&&(wChairID==GetMeChairID())) 
				{
					if(m_GameClientView.m_btCatch.IsWindowVisible()==TRUE)
						OnCallBanker(0,0);
					else 
						OnAddScore(0,0);
				}
	...
	case GS_WK_PLAYING:		//游戏状态
		{
			...
			WORD wTempCount = (INT)pow(2.0,pStatusPlay->wOutBombCount);
...
bool CGameClientDlg::OnSubCallBanker(const void * pBuffer, WORD wDataSize)
{
	...
	if(wMeChairID==pCallBanker->wCurrentUser && !IsLookonMode())
	{
		//显示控件
		ActiveGameFrame();
		if (m_cbCallBankTimes == 3)
		{
			// 最后一家闷抓
			m_GameClientView.OnBackCatch();
		}else
		{
			m_GameClientView.m_btBackCatch.ShowWindow(SW_SHOW);
			m_GameClientView.m_btLookCard.ShowWindow(SW_SHOW);
...
bool CGameClientDlg::OnSubSendCard(const void * pBuffer, WORD wDataSize)
{
	...
	m_bIsBackCatch = false;
	m_cbCallBankTimes = 0;
	m_bLookAndGiveup = false;
	return true;
}
...
bool CGameClientDlg::OnSubLandScore(const void * pBuffer, WORD wDataSize)
{
	...
	switch(pLandScore->bLandScore)
	{
		...
		case MING_CATCH:
			{
				m_bLookCard = true; // 如果是明抓，设置为已经看牌
  				{
					DispatchUserCard(m_bHandCardData,m_bHandCardCount);
					m_GameLogic.SortCardList(m_bHandCardData,m_bHandCardCount,ST_ORDER);
					DoncludeDispatchCard();
		...
		case CALL_BANKER:
			{
				...
				if(m_GameLogic.AnalyseIsBiDa(m_bHandCardData,m_bHandCardCount))
				{
 					m_GameClientView.OnCatch();
				}else{
 					m_GameClientView.m_btCatch.ShowWindow(SW_SHOW);
					m_GameClientView.m_btNoCatch.ShowWindow(SW_SHOW);
 				}
		...
		case CALL_SCORE:
			{
				...
				bool autoAddScore = false; // 自动选择倒牌标志
				if(m_wLandUser==wMeChairID)
				{
					m_GameClientView.m_btAddScore.SetButtonImage(IDB_BT_BANKER_ADD1,hInstance,false);	// 庄家拉
					m_GameClientView.m_btNoAddScore.SetButtonImage(IDB_BT_BANKER_NO_ADD1,hInstance,false);// 庄家不拉
				}
				else
				{
 					if(m_GameLogic.AnalyseHave2KingOr4Two(m_bHandCardData,m_bHandCardCount) && !m_bLookAndGiveup && m_bLookCard) // 有双王 或者 4个 2，并且没有 看牌不抓牌，已经看牌
 					{
 						m_GameClientView.OnAddScore(); // 自动倒牌
						autoAddScore = true;
  					}else
					{
						m_GameClientView.m_btAddScore.SetButtonImage(IDB_BT_ADD1,hInstance,false);	// 闲家倒
						m_GameClientView.m_btNoAddScore.SetButtonImage(IDB_BT_NO_ADD1,hInstance,false);// 闲家不倒
 					}
  				}
 
				if(m_bLookAndGiveup)// 看牌不抓，不能跟倒
				{
					// TODO :
  					m_GameClientView.OnNoAddScore();// 不倒
					autoAddScore = true;
  				}
 				
				if(!autoAddScore) // 没有自动选择倒，显示控件
				{
					m_GameClientView.m_btAddScore.ShowWindow(SW_SHOW);
					m_GameClientView.m_btNoAddScore.ShowWindow(SW_SHOW);
				}
	...
	case CALL_SCORE:
		{
			if(m_wLandUser==wCurrentUser)
				m_GameClientView.SetUserWaitType(4);
			else
				m_GameClientView.SetUserWaitType(3);
			break;
		}
	case CALL_TWO_SCORE:
		{
			if(m_wLandUser==wCurrentUser)
				m_GameClientView.SetUserWaitType(6);
			else
				m_GameClientView.SetUserWaitType(5);
			break;
		}
	}
...
bool CGameClientDlg::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	if(pGameStart->bBackCard[0]!=0)
		CopyMemory(m_bBackCard,pGameStart->bBackCard,sizeof(m_bBackCard));
	if(!IsLookonMode())m_GameClientView.m_btSortCard.EnableWindow(TRUE);


	printf("m_bCardCount[m_wLandUser]: %d \n",m_bCardCount[m_wLandUser]);
	//设置控件
	if(m_bCardCount[m_wLandUser]<20)
	{
		...
		//BYTE cbBACK[3] = {0,0,0};
		//m_GameClientView.m_BackCardControl.SetCardData(cbBACK,CountArray(cbBACK));
		//m_GameClientView.m_BackCardControl.SetDisplayFlag( true );



		//if (m_bIsBackCatch && wMeChiarID != m_wLandUser)
		//{
		// 	BYTE cbBACK[3] = {0,0,0};
		//	m_GameClientView.m_BackCardControl.SetCardData(cbBACK,CountArray(cbBACK));
		//	m_GameClientView.m_BackCardControl.SetDisplayFlag( true );
 	//	}else
		...
		if (m_wLandUser==GetMeChairID())
		{
			...
			CopyMemory(&m_bHandCardData[bCardCound],m_bBackCard,sizeof(m_bBackCard));
 			...
 			m_GameLogic.SortCardList(m_bHandCardData,m_bHandCardCount,ST_ORDER);
			m_GameLogic.SortCardList(m_bHandCardData,m_bHandCardCount,ST_COUNT);
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	//InsertShtickString( TEXT( " 为了维护网站运行以及租用服务，每局将要扣除5个游戏币" ), RGB( 255, 0, 0 ), true );
	InsertSystemString("为了维护网站运行以及租用服务，每局将要扣除5个游戏币");
...
LRESULT CGameClientDlg::OnLookOrCatch(WPARAM wParam, LPARAM lParam)
{
	...
	if(LandScore.bLandScore == LOOK_CARD )
	{
		m_bLookCard = true;
...
bool CGameClientDlg::OnSubGrantScore( const void *pBuffer, WORD wDataSize )
{
	...
	ASSERT(wDataSize==sizeof(CMD_S_Message));
	if (wDataSize!=sizeof(CMD_S_Message)) return false;


	//消息处理
	CMD_S_Message * plGrantScore=(CMD_S_Message *)pBuffer;
 	InsertSystemString(plGrantScore->message);

	ShowInformationEx(plGrantScore->message,0,MB_ICONINFORMATION,TEXT("高县斗地主"));
	return true;
}
