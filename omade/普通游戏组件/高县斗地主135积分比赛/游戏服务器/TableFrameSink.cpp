	...
	case GER_NORMAL:		//常规结束
		{
  		...
  		//			&&GameEnd.lGameScore[i] < 0L && 
				...
				if( GAME_GENRE_LONGMATCH == m_pGameServiceOption->wServerType 
					&&pUserItem->GetUserScore()->lGameGold>0 ) // 
				{
					// 负分情况,不应该允许这种情况发生
					//GameEnd.lGameScore[i] = -pUserItem->GetUserScore()->lGameGold;
					//pUserItem->GetUserScore()->lGameGold-=5;
				}
				...
				if (m_pGameServiceOption->wServerType==GAME_GENRE_SCORE ||
					m_pGameServiceOption->wServerType==GAME_GENRE_LONGMATCH ||
