...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("高县斗地主游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
...
#else
	lstrcpyn(m_GameServiceAttrib.szDataBaseName,TEXT( "QPLandSCMatchDB" ),CountArray(m_GameServiceAttrib.szDataBaseName));	

	//lstrcpyn(m_GameServiceAttrib.szDataBaseName,TEXT( "QPLandSCDB" ),CountArray(m_GameServiceAttrib.szDataBaseName));	
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("LandSC.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("LandSCMServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
#endif
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	if (pGameServiceOption->wServerType==GAME_GENRE_GOLD)
	{
		...
		pGameServiceOption->lLessScore=  pGameServiceOption->lLessScore;
		// oma set the lessScore from  the config UI in serviceload.exe UI
