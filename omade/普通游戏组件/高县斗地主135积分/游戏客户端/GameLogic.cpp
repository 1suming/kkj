...
bool CGameLogic::AnalyseIsBiDa(BYTE const cbHandCardData[],BYTE cbHandCardCount)
{
	...
	ASSERT(cbHandCardCount==17 && cbHandCardData != NULL ) ;
	if(cbHandCardCount!=17 && cbHandCardData==NULL) 
		return false;

	int	DingDingCount = 0;
	bool isBigKing = false;
	bool isSmallKing = false;

	BYTE tmpCardCount[14];

	ZeroMemory(tmpCardCount,sizeof(tmpCardCount));
	for(int i = 0;i<cbHandCardCount;++i)
	{

		BYTE cardvalue = GetCardValue(cbHandCardData[i]);
		int vtmp = 0;
		for (int k = 0;k<4;k++)
		{
			if((cardvalue>>k)&0x01)
	...
	// 3个以上的2 和双王必打
	if(tmpCardCount[2] >= 3 || (isBigKing && isSmallKing))
	{
		printf(TEXT("2的个数tmpCardCount[2]：%d\n"),tmpCardCount[2]);

		printf(TEXT("有双王或者三个以上2\n"));
	...
	for(int k = 0;k<13;k++)
	{
		if(tmpCardCount[k] == 4 && k !=1) // 跳过2 的计算
			++DingDingCount;
	}

	// 2 的个数
	DingDingCount+=tmpCardCount[2];
...
bool CGameLogic::AnalyseHave2KingOr4Two(BYTE const cbHandCardData[],BYTE cbHandCardCount)
{
	...
	// 4个2 和双王 必打,必倒
	if(tmpCardCount[2] == 4 || (isBigKing && isSmallKing))
	{
 		return true;
	}
