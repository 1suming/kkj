	...
	case IDI_CALL_SCORE:	//用户叫分
		{
			...
			if (m_cbCurrentLandScore == CALL_BANKER)
			{
				if(m_GameLogic.AnalyseIsBiDa(m_cbHandCardData,m_cbHandCardCount))
				{
					CallScore.bLandScore = MING_CATCH;
				}
			}else if(m_cbCurrentLandScore == CALL_SCORE)
 			{
 				if(m_GameLogic.AnalyseHave2KingOr4Two(m_cbHandCardData,m_cbHandCardCount))
				{
					CallScore.bLandScore = DOUBLE_SCORE;
				}
			}
...
bool CAndroidUserItemSink::OnSubCallBanker(void * pData, WORD wDataSize)
{
	...
	if (m_pIAndroidUserItem->GetChairID()==pCallBanker->wCurrentUser)
	{
		...
		if (m_cbCallBankTimes == 3)
		{
			...
			//发送数据
			CMD_C_LandScore LandScore;
			LandScore.bLandScore= BACK_CATCH;
			m_pIAndroidUserItem->SendSocketData(SUB_C_LAND_SCORE,&LandScore,sizeof(LandScore));
