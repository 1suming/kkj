...
bool __cdecl CTableFrameSink::OnActionUserSitDown(WORD wChairID, IServerUserItem * pIServerUserItem, bool bLookonUser)
{
	...
	//for (int k = 0;k<GAME_PLAYER;k++)
	{
		...
		TCHAR printmsg[512] = TEXT("");
		_snprintf(printmsg,sizeof(printmsg),TEXT("当局游戏开始用户分数：%d ,当前送分次数：%d ：wChairID: %d "),
			totalScore,pUserItem->GetUserScore()->lGrantCount,wChairID);
		CTraceService::TraceString(printmsg,TraceLevel_Debug);
			...
			&& pUserItem->GetUserScore()->lGrantCount > 0 )
		{
			...
			//__asm{int 3}
			TCHAR msg[512] = TEXT("");
			_snprintf(msg,sizeof(msg),TEXT("你的分不够了哦，亲，%d 分送给你哦！亲,剩余送分次数：%d ！加油哦"),
				GRANT_SCORE_VALUE,--pUserItem->GetUserData()->UserScoreInfo.lGrantCount);
			CTraceService::TraceString(msg,TraceLevel_Debug);
			...
			m_pITableFrame->SendTableData(wChairID,SUB_S_GRANT_SCORE,&GrantScore,sizeof(GrantScore));
			//写入积分
			m_pITableFrame->WriteUserScore(wChairID,GRANT_SCORE_VALUE,0,enScoreKind_Win);// 扣税0
		}
