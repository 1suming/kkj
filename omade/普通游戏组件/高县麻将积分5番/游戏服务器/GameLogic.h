...
#define WIK_LEFT					0x01								//左吃类型 提牌
#define WIK_CENTER					0x02								//中吃类型 贴牌
...
#define WIK_TI_PAI					0x01								//提牌 占用左吃类型
...
#define CHR_DA_DAN_DIAO   		    0x00010000									//大单吊
#define CHR_DA_DAN_DIAO_PAO   		0x00020000									//大单吊
#define CHR_WITH_THREE_PAI_JIN		0x00040000									//3个牌精加番
#define CHR_WITHOUT_TING_YONG		0x00080000									//没有听用

#define CHR_GENG_COUNT_ONE   		0x00100000									// 根数
#define CHR_GENG_COUNT_TWO   		0x00200000									//
#define CHR_GENG_COUNT_THREE		0x00400000									//
#define CHR_GENG_COUNT_FOUR			0x00800000									//
...
public:
	...
	//贴牌判断
	BYTE EstimateTiePaiCard(const BYTE cbCardIndex[MAX_INDEX], BYTE cbCurrentCard);
	// 无听用判断
	bool HaveMagicCard(const BYTE cbCardIndex[MAX_INDEX],const tagWeaveItem WeaveItem[], BYTE cbWeaveCount);
	//3牌精判断
	bool HaveThreePaiJin(const BYTE cbCardIndex[MAX_INDEX],const tagWeaveItem WeaveItem[], BYTE cbWeaveCount);
...
public:
	...
	//提牌分析
	BYTE AnalyseTIPaiCard(const BYTE cbCardIndex[MAX_INDEX],const tagWeaveItem WeaveItem[], BYTE cbWeaveCount,tagTICardResult& TICardResult);
	//杠牌分析
...
protected:
	...
	// 大单调
public:
	bool IsDaDanDiao(const BYTE cbCardIndex[MAX_INDEX],const BYTE cbWeaveCount);
