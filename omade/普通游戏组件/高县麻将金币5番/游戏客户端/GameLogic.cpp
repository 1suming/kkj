...
WORD CGameLogic::GetChiHuActionRank(const CChiHuRight & ChiHuRight)
{
	...
	if( !(ChiHuRight&CHR_GANG_SHANG_PAO).IsEmpty() )
		wFanShu *= 2;
	//牌精加番
	if( !(ChiHuRight&CHR_WITH_THREE_PAI_JIN).IsEmpty() )
		wFanShu *= 2;
	//无听用加番
	if( !(ChiHuRight&CHR_WITHOUT_TING_YONG).IsEmpty() )
		wFanShu *= 2;
 	return wFanShu;
}
...
BYTE CGameLogic::AnalyseTIPaiCard(const BYTE cbCardIndex[MAX_INDEX],const tagWeaveItem WeaveItem[], BYTE cbWeaveCount,tagTICardResult& TICardResult)
{
	...
	//   提牌, 只判断可以提牌，具体提法在CLIENT 端实现
	BYTE ret = WIK_NULL;
	if (m_cbMagicIndex!=MAX_INDEX)
	{
			for (BYTE j = 0; j<cbWeaveCount;j++)
			{
				if((WeaveItem[j].cbWeaveKind&WIK_TIE_PAI)!=0 && cbCardIndex[SwitchToCardIndex(WeaveItem[j].cbCenterCard)]>=1)
				{
					// 组合类型是贴牌 ， 手上至少有一张牌是跟贴牌同花色
 					ret |=  WIK_TI_PAI;
 					TICardResult.cbCardData[TICardResult.cbCardCount++]=WeaveItem[j].cbCenterCard;
 				}
...
BYTE CGameLogic::SwitchToCardData(const BYTE cbCardIndex[MAX_INDEX], BYTE cbCardData[MAX_COUNT])
{
	...
	for (BYTE i=0;i<MAX_INDEX;i++)
	{
		if( i == m_cbMagicIndex ) continue; //   钻牌已转换，转换剩余牌，总数不变
		if (cbCardIndex[i]!=0)
		{
...
bool CGameLogic::IsHuaZhu( const BYTE cbCardIndex[], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount )
{
 ...
 bool CGameLogic::IsDaDanDiao(BYTE WeavCount)
{
	if(WeavCount == 4)
	return true;
	else
