	...
	case GER_NORMAL:		//常规结束
		{
			...
			for ( WORD i = 0; i < m_wPlayerCount; i++ )
			{
				...
				if (m_pGameServiceOption->wServerType==GAME_GENRE_SCORE || m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
				{
			...
			for (int k = 0;k<GAME_PLAYER;k++)
			{
					...
					&& pUserItem->GetUserScore()->lGrantCount > 0 )
				{
					...
					CMD_S_Message msg;
 					_snprintf(msg.message,sizeof(msg.message),TEXT("这是第 %ld 次送 %ld 分 (每天%ld 次)"),GRANT_SCORE_COUNT-leftcount,GRANT_SCORE_VALUE,
						 GRANT_SCORE_COUNT);
...
bool CTableFrameSink::UserStartOutCard()
{
	...
	for (WORD i=0;i<m_wPlayerCount;i++)
	{
		...
		//闲家倍数
		GameStart.wUserMultiple[i]=m_wBombTime*((WORD)pow(2.0,(INT)wScoreTimes));
