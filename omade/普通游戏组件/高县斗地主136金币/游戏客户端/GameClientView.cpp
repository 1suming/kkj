...
void CGameClientView::OnRButtonUp(UINT nFlags, CPoint Point)
{
	__super::OnRButtonUp(nFlags, Point); // back up
 ...
 //	//收起扑克
	...
	CWnd *pSinkWindow = NULL;
	pSinkWindow = AfxGetMainWnd();
 	//发送消息
	if (pSinkWindow!=NULL) pSinkWindow->PostMessage(IDM_RIGHT_HIT_CARD,0,0);
...
//#endif
		if(m_wUserScoreTimes[i]>0)
		{
			...
			if(m_wBombTime>1)lTemp*=(INT)(pow(2.0,m_wBombTime-1));
