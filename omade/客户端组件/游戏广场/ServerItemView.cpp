...
CServerListManager::~CServerListManager()
{
	...
	int i=0;// oma
	for (i=0;i<nCount;i++) delete m_PtrArrayType[i];// oma
...
bool CServerListManager::UpdateGameOnLineCount(DWORD dwOnLineCount)
{
	if (m_PtrArrayInside.GetCount()>0)
	{
		...
		_snprintf_s(pGameInside->szDisplayName,sizeof(pGameInside->szDisplayName),TEXT("%s [ %ld 人]"),szProductName,dwOnLineCount);
