	...
	case ItemGenre_Inside:
		{
			...
			WebBrowse(TEXT("http://205168.cqxunmei.com"),false);
	...
	case IDC_BT_BUTTON_5:				//功能按钮//OMA论坛页
		{
			...
			if(IDC_BT_BUTTON_4==nCommandID)WebBrowse(TEXT("http://205168.cqxunmei.com/user/ExChange.asp"),true);
			if(IDC_BT_BUTTON_5==nCommandID)WebBrowse(TEXT("http://205168.cqxunmei.com/bbs/index.asp?action=frameon"),true);
...
bool CGameFrame::UpdateSkinResource()
{
	...
	//设置图片
	int i=0;// oma
	for (i=0;i<CountArray(m_pRoomViewItem);i++)// oma
	{
...
void CGameFrame::RectifyControl(int nWidth, int nHeight)
{
	...
	//更换皮肤
	int i=0;// oma
	for (i=0;i<CountArray(m_btViewItem);i++)// oma
	{
		if(m_pActiveViewItem==m_pRoomViewItem[i])break;
	}
	CSkinButton * pTempButton[]={&m_btViewItem[0],&m_btViewItem[1],&m_btViewItem[2],&m_btGamePlaza};
...
int CGameFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_BrowerAD.Navigate(TEXT("http://205168.cqxunmei.com/AD/GamePlazaAD.asp"),NULL,NULL,NULL,NULL);
...
//按键测试
//UINT CGameFrame::OnNcHitTest(CPoint Point)// oma 
LRESULT CGameFrame::OnNcHitTest(CPoint Point)// oma use LRESULT
{
	//按钮测试
	if (m_bMaxShow==false)
	{
